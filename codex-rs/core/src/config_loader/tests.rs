...
async fn project_layers_prefer_closest_cwd() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".tacodex")).await?;
    tokio::fs::create_dir_all(project_root.join(".tacodex")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;

    tokio::fs::write(
        project_root.join(".tacodex").join(CONFIG_TOML_FILE),
        "foo = \"root\"\n",
    )
    .await?;
    tokio::fs::write(
        nested.join(".tacodex").join(CONFIG_TOML_FILE),
        "foo = \"child\"\n",
    )
    .await?;
    ...
    assert_eq!(project_layers[0].as_path(), nested.join(".tacodex").as_path());
    assert_eq!(
        project_layers[1].as_path(),
        project_root.join(".tacodex").as_path()
    );
...
"#;
    tokio::fs::write(project_root.join(".tacodex").join(CONFIG_TOML_FILE), root_cfg).await?;
    tokio::fs::write(nested.join(".tacodex").join(CONFIG_TOML_FILE), nested_cfg).await?;
    tokio::fs::write(
        project_root.join(".tacodex").join("root.txt"),
        "root instructions",
    )
    .await?;
    tokio::fs::write(
        nested.join(".tacodex").join("child.txt"),
        "child instructions",
    )
    .await?;
    ...
    assert_eq!(
        vec![&ConfigLayerEntry {
            name: super::ConfigLayerSource::Project {
                dot_codex_folder: AbsolutePathBuf::from_absolute_path(project_root.join(".tacodex"))?,
            },
            config: TomlValue::Table(toml::map::Map::new()),
            version: version_for_toml(&TomlValue::Table(toml::map::Map::new())),
        }],
